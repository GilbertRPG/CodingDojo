# Word Count
# Given the text file SenseAndSensibilityCh1.txt, you are to find out how many times each word in the text file occurs. The output should be a Python dictionary where the keys are unique words in the novel and the values are how many times each key occurred. Below is part of a sample output (don't worry if your answer isn't the exact same as your answer will vary depending on how you remove punctuation and such). 
...
# 'settled': 1,
# 'in': 29,
# You are not to import any libraries for this task. In programming there are a lot of ways to accomplish a task. This exercise is meant to illustrate how much harder it often is to solve a problem from scratch without the aid of Python libraries. Don't worry if you have trouble solving this problem. 


# Breaking the problem into simpler problems 
# Here are some suggestions of how you can make this task easier:

# a) Create a notebook in Google Colab.

# b) Copy the contents of the text file into a Colab Notebook and make it a variable.

# c) Remove punctuation from the variable.

# d) Convert the string into a word list (look for some string methods).

# e) Make a dictionary and count how many times each word occurred.

# Exercise - Word Count
# Make your best attempt at the problem!

# Turn in a .ipynb file with your code and your solution.
def wallOfText():
    return ''' The family of Dashwood had long been settled in Sussex.  Their estate
...
# this splits the whole string into a big list, extended onto itself.
def convertStringToList(string):
    li = list(string.lower().split(" ")) 
    words = []
    for i in li:
        newSanatizedList = sanitize(i)
        words.extend(newSanatizedList)
    return words

# takes input of dirty word and outputs a list of "clean" words, that have no non-alphanumeric characters
def sanitize(dirty):
    cleanWords = []
    storedLetters = []
    for c in dirty:
        if c.isalnum():
            storedLetters.append(c) # start concat for each letter to build a word
        elif len(storedLetters) != 0: # does storedLetters have NO values? if NOT, do this.
            cleanWords.append(''.join(storedLetters)) # this compacts the individual letters into words
            storedLetters = []
    if len(storedLetters) != 0: # check and see if storedLetters is clear. Happens after elif.
        cleanWords.append(''.join(storedLetters))
    return cleanWords

# 
def wordCount(text):
    wordList = convertStringToList(text) # takes the output of convertStringToList to become a list to use in the dictionary
    wordDictonary = {}
    for w in wordList:
        wordDictonary[w] = wordDictonary.get(w, 0) + 1 # this adds a count of each word to the dictonary
    return wordDictonary

print(wordCount(wallOfText()))